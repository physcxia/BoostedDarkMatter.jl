...
\begin{aligned}
    T_1^{\min}
&=
    \left(\frac{T_4}{2} - m_2\right)
    \left(1 ± \sqrt{1 + \frac{2T_4}{m_1}\frac{(m_1+m_2)^2}{(2m_2-T_4)^2}}\right)\\
&=
    \frac{T_4}{2} - m_2
    +
    \sqrt{\left(\frac{T_4}{2} - m_2\right)^2 + \frac{T_4(m_1+m_2)^2}{2m_1}}.
\end{aligned}
```

In the case of
``T_4 < 2m_2`` and ``\left(\frac{T_4}{2} - m_2\right)^2 ≫ \frac{T_4(m_1+m_2)^2}{2m_1}``,
we need to expand the square root to avoid precision loss, and the result is
```math
T_4^{\min} ≈ \frac{T_4(m_1+m_2)^2}{2m_1(2m_2 - T_4)}.
```
